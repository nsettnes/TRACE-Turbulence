	...
	SubShader 
	{
		...
		void vert (inout appdata_full v, out Input OUT)
		{
			...
			if(_Displace > 0.5)
			{
				...
				if (_Animated < 0.5)
					h = ValueBillowed(float4(OUT.pos.xyz, t), _Octaves, _Offset, _Frequency, _Amplitude, _Lacunarity, _Persistence);
				else
					h = ValueBillowed(float4(OUT.pos.xyz, t + _Time.y * _AnimSpeed), _Octaves, _Offset, _Frequency, _Amplitude, _Lacunarity, _Persistence);
				if (_Normalize > 0.5)
					h = h * 0.5 + 0.5;
		...
		void surf (Input IN, inout SurfaceOutput o) 
		{
			...
			if (_Animated < 0.5)
				h = ValueBillowed(float4(IN.pos.xyz, t), _Octaves, _Offset, _Frequency, _Amplitude, _Lacunarity, _Persistence);
			else
				h = ValueBillowed(float4(IN.pos.xyz, t + _Time.y * _AnimSpeed), _Octaves, _Offset, _Frequency, _Amplitude, _Lacunarity, _Persistence);
			if (_Normalize > 0.5)
				h = h * 0.5 + 0.5;
